...
mod tests {
    ...
    fn sort_u32_ascending() {
        let mut x: Vec<u32> = vec![10, 30, 11, 20, 4, 320, 21, 110];
        sort(&mut x, &Ascending);
        assert_eq!(x, vec![4, 10, 11, 20, 21, 30, 110, 320]);
    }
    ...
    fn sort_u32_descending() {
        ...
        assert_eq!(x, vec![320, 110, 30, 21, 20, 11, 10, 4]);
    }
    ...
    #[test]
    fn sort_students_by_age_descending() {
        let taro = Student::new("Taro", "Yamada", 16);
        let hanako = Student::new("Hanako", "Yamada", 14);
        let kyoko = Student::new("Kyoko", "Ito", 15);
        let ryosuke = Student::new("Ryosuke", "Hayashi", 17);
        ...
        let mut x = vec![&taro, &hanako, &kyoko, &ryosuke];
        let expected = vec![&ryosuke, &taro, &kyoko, &hanako];

        assert_eq!(sort_by(&mut x, &|a, b| b.age.cmp(&a.age)), Ok(()));
        assert_eq!(x, expected);
    }
}
